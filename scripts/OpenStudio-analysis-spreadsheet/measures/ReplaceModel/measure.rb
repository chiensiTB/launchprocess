...
class ReplaceModel < OpenStudio::Ruleset::ModelUserScript
  ...
  def run(model, runner, user_arguments)
    ...
    seeds_path = File.expand_path("../../../lib/seeds", current_path)
    target_path = File.expand_path("../../../seed", current_path)
    osm_path = seeds_path+"/"+external_model_name
    #copy over to the seed folder
    FileUtils.cp_r(osm_path,target_path)
    ...
    if osw_file.is_initialized
      ...
      runner.registerError("Did not find #{external_model_name} in paths described in OSW file for current working directory " + Dir.pwd.to_s)
      current_path = Dir.pwd
      target = File.expand_path("../../../lib/seeds", current_path)
      Dir.chdir(target)
      fullpath = target+"/"+external_model_name
      runner.registerError("Looking for new path at " + fullpath)
      osw_file = runner.workflow.findFile(external_model_name)
      if osw_file.is_initialized
        runner.registerError("Is Intialized.")
        external_model_name = osw_file.get.to_s
        Dir.chdir(current_path)
      else
        runner.registerError("Did not initialize.")
        return false
      end
    end
