...
currentDir = Dir.pwd
p currentDir
newDir = File.join(currentDir,"scripts","OpenStudio-analysis-spreadsheet")
p newDir

#http://stackoverflow.com/questions/3339883/to-change-directory-inside-a-ruby-script
Dir.chdir(newDir)

Bundler.setup

output = `bundle exec rake console`
puts output

#https://makandracards.com/konjoot/21173-run-rake-tasks-from-ruby-script
rake = Rake.application
rake.init
rake.load_rakefile
p rake.original_dir
p "Rakefile is loaded"
p rake.tasks()
...
# excel is the excelfile location.  Textfile is the textfile location.
# chiensiTB added this a a bootstrapping mechanism that does not require passing in additional ... nix this idea
...
# desc " :project is a path to where the xlsx file is located"
# task :run_custom, [:target, :project, :download, :batch_run_method] do |t, args|
#   puts "Target: ", :target.to_s
#   puts "Project: ", :project.to_s
#   puts "Download: ", :download.to_s
#   puts "Batch method: ", :batch_run_method.to_s
